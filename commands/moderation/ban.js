...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        // No args
        if (!args[0]) {
            return message.reply("Merci de mettre un utilistateur à exclure ..")
                .then(m => m.delete(5000));
        }

        // No reason
        if (!args[1]) {
            return message.reply("Merci de mettre une raison à votre sanction ..")
                .then(m => m.delete(5000));
        }
        ...
        if (toKick.id === message.author.id) {
            return message.reply("Vous ne pouvez exclure vous-même...")
                .then(m => m.delete(5000));
        }
        ...
        const promptEmbed = new RichEmbed()
            ...
            .setDescription(`Voulez-vous exclure ${toKick}?`)
        ...
        await message.channel.send(promptEmbed).then(async msg => {
            ...
            if (emoji === "✅") {
                ...
                toKick.kick(args.slice(1).join(" "))
                    .catch(err => {
                        if (err) return message.channel.send(`Le kick ne marche pas sûrement dû à ${err}`)
                    });
